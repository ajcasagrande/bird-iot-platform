...
class SerialReader(StoppableThread):
    ...
    """
    def __init__(self, serial, event_queue, auto_reset):
    ...
    def run(self):
        if not self.serial.is_open:
            ...
            # self.serial.dtr = IO0_HIGH
            # self.serial.rts = EN_HIGH

            self.serial.open()

            if self.auto_reset:
                # Force an RTS reset on open
                self.serial.dtr = IO0_HIGH
                self.serial.rts = EN_LOW
                time.sleep(0.1)
                self.serial.rts = EN_HIGH
        try:
            while self.alive:
...
class Monitor(object):
    ...
    """
    def __init__(self, serial_instance, elf_file, print_filter, make="make", toolchain_prefix=DEFAULT_TOOLCHAIN_PREFIX, eol="CRLF", auto_reset=False):
        ...
        self.serial_reader = SerialReader(self.serial, self.event_queue, auto_reset)
    ...
    def handle_menu_key(self, c):
        if c == self.exit_key or c == self.menu_key:  # send verbatim
        ...
        elif c == CTRL_J:  # Recompile & upload via serial
            self.run_make("flash-serial")
        elif c == CTRL_O:  # Recompile & upload via OTA
            self.run_make("flash")
            ...
            self.serial.dtr = IO0_HIGH  # IO0=HIGH
            self.serial.rts = EN_LOW   # EN=LOW, chip in reset
            time.sleep(1.3)  # timeouts taken from esptool.py, includes esp32r0 workaround. defaults: 0.1
            self.serial.dtr = IO0_LOW   # IO0=LOW
            self.serial.rts = EN_HIGH  # EN=HIGH, chip out of reset
            time.sleep(0.45)  # timeouts taken from esptool.py, includes esp32r0 workaround. defaults: 0.05
            self.serial.dtr = IO0_HIGH  # IO0=HIGH, done
        else:
...
def main():
    ...
    parser.add_argument(
        '--reset',
        help="Auto reset board using RTS",
        dest='auto_reset',
        action='store_true',
        default=False)
    ...
    monitor = Monitor(serial_instance, args.elf_file, args.print_filter, args.make, args.toolchain_prefix, args.eol, args.auto_reset)
